library(readxl)
Windsor <- read_excel("Desktop/University_of_Leeds/Project/Windsor 2700TH flow and stage data 2013-14.xlsx", 
...
hmW=4.8
QmW=300 
# 250 and 300 given by [23]
# Checked by looking at given data to find an approx. value; raw data given use of ultrasound [22]
...
hmW_sc=(hmW-minStW)/(maxStW-minStW)
QmW_sc=(QmW-minFlW)/(maxFlW-minFlW)
...
segments(-hmW_sc, QmW_sc, 1, QmW_sc, lty=3) 
segments(-hmW_sc, -1, -hmW_sc, QmW_sc, lty=3)
...
segments(ScVecDW[1], QtW_sc, ScVecDW[1], QmW_sc, lty=1, lwd=2)  # FEV box
segments(ScVecDW[lSVD], QtW_sc, ScVecDW[lSVD], QmW_sc, lty=1, lwd=2) 
segments(ScVecDW[1], QtW_sc, ScVecDW[lSVD], QtW_sc, lty=1, lwd=2) 
segments(ScVecDW[1], QmW_sc, ScVecDW[lSVD], QmW_sc, lty=1, lwd=2) 
...
text(1.1, QmW_sc, expression("Q"[m])) 
text(-hmW_sc, -1.1, expression("h"[m]))
text(ScVecDW[1], -0.4, expression("h"[t]), cex=1) 
text(0.55, -0.4,labels = "= 4.3 m", cex=1)
text(ScVecDW[1], -0.55, expression("Q"[t]), cex=1)
text(0.545, -0.55, expression("= 250 m" ^ "3"~"/s"), cex=1) 
text(ScVecDW[1], -0.75, expression("h"[m]), cex=1) 
text(0.55, -0.75,labels = "= 4.8 m", cex=1)
text(ScVecDW[1], -0.9, expression("Q"[m]), cex=1)
text(0.54, -0.9, expression("= 300 m" ^ "3"~"/s"), cex=1) 
